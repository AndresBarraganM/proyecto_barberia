      ...
      <div class="agendar-grid">
        ...
        <div class="card">
          ...
          <div
            ...
            :class="[
              'stylist-item',
              { active: estilistaSeleccionado?.id === e.id },
            ]"
    ...
    <div v-if="mostrarPago" class="modal-overlay">
      <div class="modal">
        ...
        <div class="payment-options">
          <button class="btn-option" @click="pagoEfectivo">💵 Efectivo</button>

          <button class="btn-option" @click="pagoTarjeta">💳 Tarjeta</button>
        </div>
      </div>
    </div>
...
watch(fecha, async (nuevaFecha) => {
  ...
  try {
    const res = await axios.get(`${API_URL}/recepcionista/horarios`, {
      params: { fecha: nuevaFecha },
      headers: { Authorization: `Bearer ${token}` },
    });
    horas.value = res.data.horas;
...
const cargarEstilistas = async () => {
  try {
    const res = await axios.get(`${API_URL}/recepcionista/estilistas`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    estilistas.value = res.data.estilistas;
  } catch (error) {
...
const validar = () => {
  ...
  if (!horaSeleccionada.value)
    return ((errorMessage.value = "Selecciona hora"), false);
  if (!estilistaSeleccionado.value)
    return ((errorMessage.value = "Selecciona estilista"), false);
  return true;
};
...
const confirmarEfectivo = async () => {
  try {
    ...
    */
    ...
    console.error(error);
    errorMessage.value = "Error al crear la cita";
  }
};

const pagoTarjeta = async () => {
  try {
    await axios.post(
      `${API_URL}/citas`,
      {
